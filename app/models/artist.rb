class Artist < ApplicationRecord
  ...
  def death_date_is_valid
    errors.add(:birth_date, "must exist when death date is present") if death_date.presence? && birth_day.blank?
  end

  def death_date_is_after_birth_date
    errors.add(:death_date, "must be after birth date") if death_date < birth_date
  end
end
